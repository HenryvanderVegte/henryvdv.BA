...
The idea of selecting that approach is as follows: A human would reject all intermediate noun phrases due to several indicators that exclude the candidate for some reason. In contrast, noun phrases earlier in the text than the antecedent could be legitimate candidates if the antecedent did not exist. In conclusion, a valid rejection of candidates preceding the antecedent is only possible if the antecedent is already detected. However, if the antecedent is already detected no further examination needs to be done. With the algorithm of \cite{soon2001machine}, 904 positive and 2164 negative were created in total.

The anaphora set consists of 17 reflexives (herself, himself, themselves, itself), 379 nominatives (he, she, they), and 508 possessives (their, its, her, his). An ''it" appearing in structures like "it is raining" is considered pleonastic. There are several ways of dealing with that non-referential structures, leading from machine learning-based \citep{boyd2005identifying} to rule-based approaches \citep{lappin1994algorithm}. Several pronoun resolution approaches decided to manually exclude pleonastic pronouns \citep{kennedy1996anaphora,bergsma2005automatic}. As the number of occurences of ''it" is not decisive, it will be neglected in this work.
